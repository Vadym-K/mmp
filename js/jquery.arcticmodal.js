...
(function($) {
	...
	$('input#bonus-phone').unbind().blur( function(){       
        var id = $(this).attr('id');
        var val = $(this).val();
        switch(id)
            {
    ...
    $('form#bonus-form button').on('click', function(e){

        if($('form#bonus-form .not_error').length == 1) {
            actions.close();
        } else {
            return false;
        }
	...
	$('.bonus .close-modal').on('click', function(){
		$(this).arcticmodal('close');
	});
